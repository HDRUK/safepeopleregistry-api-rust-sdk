...
/// struct for typed errors of method [`query_query`]
...
pub async fn query_query(configuration: &configuration::Configuration, x_client_id: &str, x_signature: &str, query_query_request: models::QueryQueryRequest) -> Result<models::QueryQuery200Response, Error<QueryQueryError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::QueryQuery200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::QueryQuery200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<QueryQueryError> = serde_json::from_str(&content).ok();
